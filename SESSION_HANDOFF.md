...
**작성자**: Claude Opus 4.5
**마지막 업데이트**: 2026-01-28 KST (Phase 3-3 완료)
**프로젝트 상태**: ✅ Phase 1~3 완료

**다음 마일스톤**: Phase 3-4 최종 전략 확정

---

## 🔬 Phase 3-3: Unbiased Hybrid Grid Search ✅ (2026-01-28)

### 목표

돌파 시그널(VR>=3.0, Price>=5%) 내에서 **모든 수급 요인을 평등하게 전수조사**하여 최적 조건 도출

### 분석 설계

```
입력: Rough Signal 통과 종목 (IS: 3,879건, OOS: 1,489건)

Grid Search 변수:
- 주체: [개인, 외국인, 금융투자, 연기금]
- 기간: [1, 3, 5, 10, 20, 30, 50, 60일]
- 임계치: -5% ~ +5% (0.5% 단위)
- 정밀 스캔: 금융투자 10D, 연기금 30D (0.1% 단위)

총 분석 조합: 1,664개
```

### 핵심 발견 1: 최우수 단일 수급 팩터

**Q: 돌파 상황에서 통계적으로 가장 우월한 단일 수급 팩터는?**

| 순위 | 조건 | 20D 수익률 | 승률 | 시그널 | Score |
|------|------|-----------|------|--------|-------|
| 1 | **개인 1D BUY >=0%** | **4.00%** | 47.7% | 1,007 | 13.17 |
| 2 | 개인 3D BUY >=0% | 3.45% | 47.6% | 1,121 | 11.53 |
| 3 | 외국인 1D SELL <=-5% | 3.26% | 46.7% | 1,115 | 10.69 |
| 4 | 금융투자 1D SELL <=-5% | 3.19% | 46.2% | 1,566 | 10.84 |
| 5 | 연기금 1D SELL <=-5% | 3.18% | 44.2% | 936 | 9.62 |

**핵심 인사이트:**
- **개인 순매수 (BUY)가 최강 팩터** (IS 4.00% -> OOS 8.77%)
- 임계치 0% 이상이면 충분 (강도보다 방향이 중요)
- 단기(1~3일)가 장기(50~60일)보다 효과적

### 핵심 발견 2: 선형성 vs 변곡점

**Q: 수급 강도가 높아질수록 수익률이 선형적으로 증가하는가?**

**답: 대부분 비선형 (변곡점 존재)**

| 투자자 | Duration | 방향 | 패턴 |
|--------|----------|------|------|
| 개인 | 10D | SELL | 비선형 (변곡점 존재) |
| 개인 | 10D | BUY | 비선형 (변곡점 존재) |
| 외국인 | 50D | BUY | 선형(+): 강한 BUY = 높은 수익 |
| 금융투자 | 30D | BUY | 선형(+): 강한 BUY = 높은 수익 |
| 연기금 | 10D | BUY | 선형(-): 강한 BUY = 낮은 수익 |

**결론:** 수급 강도를 무한정 높이면 수익률이 꺾임 (과열 구간 존재)

### 핵심 발견 3: 변곡점 분석 (정밀 스캔)

**금융투자 10D & 연기금 30D 0.1% 단위 분석 결과:**

| 투자자 | Duration | 방향 | 최적 임계치 | 수익률 | 승률 | 시그널 |
|--------|----------|------|------------|--------|------|--------|
| 금융투자 | 10D | SELL | **<=0%** | 2.98% | 45.4% | 1,668 |
| 금융투자 | 10D | BUY | >=0.1% | 2.29% | 45.0% | 2,171 |
| 연기금 | 30D | SELL | **<=0%** | 3.00% | 44.9% | 1,478 |
| 연기금 | 30D | BUY | >=0.1% | 2.05% | 45.6% | 2,105 |

**핵심 인사이트:**
- **임계치 0%가 변곡점** (SELL: <=0%, BUY: >=0%)
- 수급 강도(절대값)보다 **방향(부호)**이 더 중요
- "얼마나 많이"보다 "사는가 파는가"가 핵심

### OOS (2025) 검증 결과

**Top 5 IS 조건의 OOS 성과:**

| 조건 | IS 수익률 | OOS 수익률 | IS 승률 | OOS 승률 | 상태 |
|------|----------|-----------|--------|---------|------|
| 개인 1D BUY >=0% | 4.00% | **8.77%** | 47.7% | 53.4% | OK |
| 개인 1D BUY >=0.5% | 4.00% | 8.77% | 47.7% | 53.4% | OK |
| 개인 1D BUY >=1.0% | 4.00% | 8.77% | 47.7% | 53.4% | OK |
| 개인 1D BUY >=1.5% | 4.00% | 8.77% | 47.7% | 53.4% | OK |
| 개인 1D BUY >=2.0% | 4.00% | 8.77% | 47.7% | 53.4% | OK |

**결론:** 모든 Top 조건이 OOS에서 **2배 이상 성과 개선** (4.00% -> 8.77%)

### Golden Filter 확정

**Phase 3-3 전수조사 결과 확정된 최적 필터:**
...
GOLDEN_FILTER_V2 = {
    ...
    # Stage 2: Quality Filter (Phase 3-3 결과)
    'quality_filter': '개인 1D BUY',
    'investor': '개인_비중',
    'duration': 1,  # 1일 (당일)
    'direction': 'BUY',
    'flow_threshold': 0.0,  # 0% 이상 순매수면 OK
...
```
results/p3/
├── p3_03_total_grid_search_results.csv  # 1,664개 조합 전수조사
├── p3_03_optimal_thresholds.csv         # 최적 임계치 (64개)
├── p3_03_top_conditions.csv             # Top 조건
├── p3_03_oos_validation.csv             # OOS 검증 결과
├── p3_03_inflection_points.csv          # 변곡점 분석
└── p3_03_linearity_analysis.csv         # 선형성 분석
```
...
### Golden Filter 비교: Phase 3-2 vs Phase 3-3

| 항목 | Phase 3-2 | Phase 3-3 |
|------|-----------|-----------|
| 필터 | 금융투자 10D SELL | **개인 1D BUY** |
| IS 수익률 | 2.85% | **4.00%** |
| OOS 수익률 | **10.77%** | 8.77% |
| IS 승률 | 45.4% | **47.7%** |
| OOS 승률 | **54.7%** | 53.4% |
...
**결론:**
- Phase 3-2 필터 (금융투자 10D SELL): OOS 수익률 최고 (10.77%)
